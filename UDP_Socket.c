...
uint32_t udp_cb_func (int32_t socket, const  NET_ADDR *addr, const uint8_t *buf, uint32_t len) {
  // Data received
	printf("UDP buf[0]:%02X buf[1]:%02X\n",buf[0],buf[1]);
  if ((buf[0] == 0x01) && (len == 2)) {
    // Switch LEDs on and off
    vioSetSignal(vioLED0,buf[1]?vioLEDon:vioLEDoff);
  }
  return (0);
}
...
void send_udp_data (void) {
  ...
  if (udp_sock > 0) {
    ...
    NET_ADDR addr = { NET_ADDR_IP4, 1001, 192, 168, 0, 192 };
    // IPv6 address: [fe80::1c30:6cff:fea2:455e]
//  NET_ADDR addr = { NET_ADDR_IP6, 2000,
//                    0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
//                    0x1c, 0x30, 0x6c, 0xff, 0xfe, 0xa2, 0x45, 0x5e };
    char buffer[]="Hello Vives";
		uint8_t *sendbuf;
 
    sendbuf = netUDP_GetBuffer (strlen(buffer));
    memcpy(sendbuf,buffer,strlen(buffer));
 
    netUDP_Send (udp_sock, &addr, sendbuf, strlen(buffer));
  }
}
